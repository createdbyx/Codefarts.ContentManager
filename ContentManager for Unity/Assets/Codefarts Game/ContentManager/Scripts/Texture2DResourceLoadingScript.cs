...
namespace Codefarts.ContentManager.Scripts
{
    ...
    /// </summary>
    public class Texture2DResourceLoadingScript : MonoBehaviour
    {
        /// <summary>
        ...
        /// </summary>
        private string filePath = @"Codefarts.ContentManager/Textures/codefarts 256x256";

        /// <summary>
        /// Initializes a new instance of the <see cref="Texture2DResourceLoadingScript"/> class.
        /// </summary>
        public Texture2DResourceLoadingScript()
        {
        ...
        public void OnGUI()
        {
            this.filePath = GUI.TextField(new Rect(10, 10, Screen.width - 10, 20), this.filePath);
            ...
            if (GUI.Button(new Rect(10, 40, 200, 25), "Load asynchronously"))
            {
                ...
                try
                {
                    this.manager.Load<Texture2D>(this.filePath, this.SetupTextureHtmlObject, false);
                }
                catch (Exception ex)
                {
                    this.SetupTextureHtmlObject(new ReadAsyncArgs<string, Texture2D>() { Key = this.filePath, Progress = 100f, State = ReadState.Completed, Error = ex });
                }
            }
            ...
            if (GUI.Button(new Rect(220, 40, 200, 25), "Load synchronously"))
            {
                ...
                try
                {
                    var result = this.manager.Load<Texture2D>(this.filePath, false);
                    this.SetupTextureHtmlObject(new ReadAsyncArgs<string, Texture2D>() { Key = this.filePath, Progress = 100f, State = ReadState.Completed, Result = result });
                }
                catch (Exception ex)
                {
                    this.SetupTextureHtmlObject(new ReadAsyncArgs<string, Texture2D>() { Key = this.filePath, Progress = 100f, State = ReadState.Completed, Error = ex });
        ...
        public void Start()
        {
            ...
            // register readers
            this.manager.Register(new Texture2DResourceReader());
        ...
        /// </param>
        private void SetupTextureHtmlObject(ReadAsyncArgs<string, Texture2D> data)
        {
